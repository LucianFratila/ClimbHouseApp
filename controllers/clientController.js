...
exports.createClimber = catchAsync(async (req, res, next) => {
  const client = await Client.findById({ _id: req.params.ownerId }).select(
    "name + status + timeOut + finalTime + timeIn"
  );
...
exports.timeResume = catchAsync(async (req, res, next) => {
  ...
  timeResume.totalPaused = (totalPausedforTotal.totalPaused + (Date.now() - clientResume.paused) / 1000 / 60).toFixed(
    0
  );
...
exports.timeEnd = catchAsync(async (req, res, next) => {
  ...
  const clientLogHistory = await Client.findOneAndUpdate(
    { _id: req.params.id },
    {
      $addToSet: {
...
exports.reset = catchAsync(async (req, res, next) => {
  ...
  const clientP = await Client.updateOne(
    { _id: req.params.id },
    {
      $unset: {
        prodHistory: prodsBackEnd,
  ...
  if (climbers[0].status === true) {
    try {
      ...
      if (climbers[0].kid === true) {
        ...
        if (y >= 35.0001 && y <= 125) {
          aftertimeEnd.due = a * (c - 15 + Math.ceil((y - 35) / 15) * 7.5); // pret final rotunjit in sus
        }
        if (y >= 125.0001) {
...
exports.endIndividualTest = catchAsync(async (req, res, next) => {
  ...
  let c = kidPrice; //pret copii
  let d = adultPrice; //pret adulti
  let y = req.params.time; //timp scurs
  let kid = a * (c - 10);
  let adult = b * (d - 10);
  let kidOver35min = a * (c + Math.ceil((y - 35) / 15) * 5); // pret final rotunjit in sus
  let adultOver35min = b * (d + Math.ceil((y - 35) / 15) * 5); // pret final rotunjit in sus
  ...
  if (true) {
    ...
    if (y >= 35.0001 && y <= 125) {
      aftertimeEnd.due = a * (c - 15 + Math.ceil((y - 35) / 15) * 7.5);
    }
    if (y >= 125.0001) {
      aftertimeEnd.due = a * (c + 0 + Math.ceil((y - 35) / 15) * 5);
    }
  }
