    ...
    >
      <Modal.Header>
        <Modal.Title id='contained-modal-title-vcenter'>
          <h6>
            Session History for {props.name} | Results: {props.data.length} | Created at:{" "}
            {props.date.toString().substr(0, 10)}
          </h6>
          <p style={{ fontSize: "14px" }}>
            <FaTimes color='red' size='12px' /> kicked of | <FaPlay color='green' size='12px' /> Session ok | k: kids /
            a: adults{" "}
          </p>
        </Modal.Title>
      </Modal.Header>
                      ...
                      <ListGroup key={item._id} variant='flush'>
                        <ListGroup.Item variant='info'>
                          <span style={{ verticalAlign: "middle", display: "inline-block" }}>
                            {item.kickedOF ? (
                              <span>
                                <FaTimes color='red' size='14px' />
                              </span>
                            ) : (
                              <span>
                                <FaPlay color='green' size='14px' />
                              </span>
                            )}{" "}
                            <b>{item.timeIn}</b> | {item.admin} | <FaClock size='14px' /> {item.totalTime}/min |{" "}
                            <FaDollarSign size='14px' /> {item.due}/lei @ k: {item.kids}/a: {item.adults}
                          </span>
                        </ListGroup.Item>
                      </ListGroup>
                    ))
...
function CreateClimberModal(props) {
  ...
  return (
    ...
    >
      <Modal.Header>
        <Modal.Title id='contained-modal-title-vcenter'>Insert new Climber</Modal.Title>
      </Modal.Header>
      <Modal.Body>
        <Container>
          <InputGroup>
            <Form inline onSubmit={onSubmit}>
              <Form.Group>
                ...
                <Form.Group controlId='exampleForm.ControlSelect1'>
                  <Form.Label style={{ marginLeft: "5px" }}> / </Form.Label>
                  <Form.Control onChange={onChangeClimber} style={{ width: "120px", marginLeft: "5px" }} as='select'>
                    <option defaultValue='selected' value='adult'>
                      adult
                    </option>
                    <option value='kid'>kid</option>
                    <option value='minikid'>minikid</option>
                  </Form.Control>
                </Form.Group>
...
function EditClientProvisional(props) {
  ...
  const refreshFunction = () => {
    setPostStop(!postStop ? true : false);
  };
  let trasnfer = (a) => {
    // console.log(a);
    return a;
  ...
  const toggleTerms = async (id) => {
    try {
      const url = `${SERVER_URL}/clients/termstoggle/${id}`;

      await axios({
        method: "POST",
        url: url,
      }).then((res) => setTerms(res.data.client.terms));
    } catch (error) {
      alert(error);
    }
  };

  function onChangeTermsToggle(id) {
    toggleTerms(id);
  }
        ...
        <Container fluid>
          <Row>
            <h4 style={{ marginTop: "10px" }}>
              ...
              {data.client.name} - <FaEnvelope /> {data.client.email} -<FaPhone /> {data.client.phone} -{" "}
              <Button
                variant='info'
                onClick={() => {
                  setShowHistory(true);
              ...
              {data.client.terms ? (
                <span>
                  <Button
                    variant='success'
                    style={{ margin: "10px" }}
                    onClick={() => onChangeTermsToggle(data.client._id)}
                  >
                    <FaCheckCircle style={{ margin: "0 4 0 0" }} size='1.4em' />
                    Terms and Rules are signed
                  </Button>
                </span>
              ) : <span><Button
              variant='danger'
              ...
              onClick={() => onChangeTermsToggle(data.client._id)}
            >
              <FaWindowClose style={{ margin: "0 4 0 0" }} size='1.4em' />
              Terms and Rules are NOT signed
            </Button></span>}
            </h4>
          </Row>
          <Row>
            <h6>
              User: {data.client.status === true ? "Active" : "Not Active"} | <IoMdTimer size='1em' />{" "}
              {data.client.startTime === "0" ? <span>Time not started</span> : <span>{data.client.startTime}</span>}
            </h6>
          </Row>
          <Row>
            <h6>
              {data.client.kids === 0 && data.client.adults === 0 ? (
                ...
                <span>
                  {data.client.adults + data.client.kids}/{data.client.adultsRemaining + data.client.kidsRemaining}{" "}
                  climbers @ {data.client.adults} (remainig:{data.client.adultsRemaining}) adult(s) / {data.client.kids}{" "}
                  (remaining:{data.client.kidsRemaining}) kid(s)
                </span>
              )}{" "}
              |
              {data.client.finalTime > 0 ? (
                <span>
                  {" "}
                  Final: {data.client.finalTime} min @ Due: {dueTotal} /lei
                </span>
              ) : (
                <span>
                  {" "}
                  <IoMdTimer size='1em' /> {calculateMins(Date.now(), data.client.timeIn)} /min
                </span>
              )}
            </h6>
          </Row>
              ...
              <span>
                <span>
                  ...
                  <span style={{ marginLeft: "5px", fontSize: "25px" }}>
                    Total due for stopped time : <b>{dueTotal}</b> lei
                  </span>
                </span>
                ...
                <span style={{ float: "right" }}>
                  <ResetButton
                    ...
                    in={data.client.timeIn}
                    out={data.client.timeOut}
                    refresh={refreshFunction}
                    paused={data.client.pausedStatus}
                    due={data.client.due}
                    name={data.client.name}
                    time={data.client.finalTime}
                  />
                </span>
              </span>
            </Col>
                  ...
                  <Card.Body>
                    <h4>
                      {item.status === true ? (
                        <span>
                          {" "}
                          <IoMdTimer size='1em' /> {calculateMins(Date.now(), item.timeIn)} min
                        </span>
                      ) : (
                        <span>
                          <IoMdStopwatch size='0.9em' /> {item.finalTime} min | $: {item.due}lei
                        </span>
                      )}
                    </h4>
          ...
          <Row xl={4} lg={3} md={2} sm={2} xs={1} style={{ backgroundColor: "black" }}>
            {data.client.prodHistory.map((item) => (
              <Col key={item._id}>
                <Card bg='secondary' style={{ width: "auto", marginTop: "10px", marginBottom: "10px" }} text='light'>
                  <Card.Header>
                    {item.productName}
                    <span style={{ float: "right" }}>
                      <DeleteProdsHistory
