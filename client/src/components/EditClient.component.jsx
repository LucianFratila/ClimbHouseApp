...
function EditClient(props) {
  ...
  return (
    <div>
      ...
      <Container>
        <Row>
          <Col style={{ backgroundColor: "#2b2b2b", display: "flex", alignItems: "center" }} sm={6}>
            <FaUserCircle size='1.4em' />
            <span style={{ marginLeft: "5px", fontSize: "30px" }}>{name}</span>
          </Col>
          <Col style={{ backgroundColor: "#2f2e2e", display: "flex", alignItems: "center" }} sm={3}>
            <FaEnvelope /> <span style={{ marginLeft: "5px" }}>{email}</span>
          </Col>
          <Col style={{ backgroundColor: "#2f2e2e", display: "flex", alignItems: "center" }} sm={3}>
            <FaPhone /> <span style={{ marginLeft: "5px" }}>{phone}</span>
          </Col>
        </Row>
        <Row xl={4} lg={4} md={2} sm={1} xs={1} style={{ backgroundColor: "#404040" }}>
          <Col style={{ display: "flex", alignItems: "center" }} sm>
            <IoMdWalk size='1em' />
            <span style={{ marginLeft: "5px" }}> User: {status === true ? "Active" : "Not Active"}</span>
          </Col>
          <Col style={{ display: "flex", alignItems: "center" }} sm>
            <IoMdTimer size='1em' />
            <span style={{ marginLeft: "5px" }}>
              {startTime === "0" ? <span>Time not started</span> : <span>{startTime}</span>}
            </span>
          </Col>
          <Col style={{ display: "flex", alignItems: "center" }} sm>
            <FaUsers size='1em' />
            <span style={{ marginLeft: "5px" }}>
              {kids === 0 && adults === 0 ? (
                ...
                <span>
                  {adults + kids}/ @ {adults} adult(s) / {kids} kid(s)
                </span>
              )}
            </span>
          </Col>
          <Col style={{ display: "flex", alignItems: "center" }} sm>
            ...
            <span style={{ marginLeft: "5px" }}>
              {finalTime > 0 ? (
                <span>
                  Final: {finalTime} min @ {due}/lei
                </span>
              ) : pausedStatus === false ? (
                <span>{ctime(totalPaused, calculateMins(Date.now(), timeIn))}/min</span>
              ) : (
                <span>Paused: {elapsedOnPaused}/min</span>
              )}
            </span>
          </Col>
        </Row>
        <Row xl={4} lg={4} md={2} sm={1} xs={1} style={{ backgroundColor: "#404040" }}>
          <Col style={{ display: "flex", alignItems: "center" }} sm>
            ...
            <span style={{ marginLeft: "5px" }}>Subscribed: {sub === true ? "Yes" : "No"}</span>
          </Col>
          <Col style={{ display: "flex", alignItems: "center" }} sm>
            <FaCalendarCheck size='1em' />
            <span style={{ marginLeft: "5px" }}>
              {sub === true ? (
                <span>
                  {initialSub}/{remainigSub} entries remaining
                </span>
              ) : (
                "n/a"
              )}
        ...
        <Row>
          <Col style={{ backgroundColor: "#2f2e2e", display: "flex", alignItems: "center" }}>
            <FaTools size='1.4em' />{" "}
            <span style={{ marginLeft: "5px", fontSize: "20px", padding: "10px" }}>Time/Subscription Control</span>
            <Button
              variant='info'
              onClick={() => {
                setShowHistory(true);
        ...
        <Row>
          <Col style={{ backgroundColor: "#2f2e2e", display: "flex", alignItems: "center" }}>
            <FaCartPlus size='1.4em' />{" "}
            <span style={{ marginLeft: "5px", fontSize: "20px", padding: "10px" }}>Add Products</span>
          </Col>
        </Row>
        ...
        <Row xl={4} lg={3} md={2} sm={2} xs={1} style={{ backgroundColor: "#404040" }}>
          {climbersList.map((item) => (
            <Col key={item._id}>
              ...
              >
                <Card.Header>
                  {item.name}-Time: {item.finalTime} min<span style={{ float: "right" }}></span>
                </Card.Header>
                <Card.Body>
                  <Card.Text>
                    <Button
                      variant='warning'
                      onClick={() => {
                        stopAdultsIndividual(item._id);
                        setClimberID(item._id);
                        refresh();
                      }}
                    >
                      Stop
